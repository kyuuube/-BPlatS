...
/** 绑定邮箱弹窗 */
export default class BindEmailModal extends Component<Props, State> {
  ...
  handleOk = () => {
    this.editForm!.validateFields(async (err: any, values: any) => {
      if (!err) {
        ...
        const [error, resp] = await errorCapture(UserService.bindEmail(values.email, values.emailCode));
